...
class xpANN(nn.Module):
    def __init__(self, n_classes=5, mode='1d'):
        super(xpANN, self).__init__()
        if mode == '2d':
            self.conv1 = basic_block(2, 16, 3)
        elif mode == '3d':
            self.conv1 = basic_block(3, 16, 3)
        else:
            self.conv1 = basic_block(1, 16, 3)

        self.conv2 = basic_block(16, 64, 3)
        #self.conv3 = basic_block(32, 64, 5)
        self.conv4 = basic_block(64, 128, 7)

        self.head = nn.Sequential(
            nn.Flatten(start_dim=1),
            nn.Linear(3072, n_classes))
...
def ANN(n_classes, mode):
    return xpANN(n_classes, mode)
