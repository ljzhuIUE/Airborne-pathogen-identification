...
class Evaluation(object):
    ...
    def __init__(self, predict, label,train_class_num,prediction_scores = None):
        ...
        self.acc_close = self._accuracy_close()
    ...
    def _accuracy_close(self) -> float:
        """
        Returns the accuracy score of the labels and predictions.
        :return: float
        """
        ...
        a= np.array(self.predict)
        b= np.array(self.label)
        print(self.train_class_num)
        
        #Calculate accuracy of training classes
        c= np.array(np.full(len(self.label),self.train_class_num))
        correct = ((a==b) & (b!=c)).sum()
        num = (b != c).sum()
        
        #----------------------Print confusion matrix-----------------------
        matric = []
        for i in range(self.train_class_num+1):
             matric.append(np.array(np.full(len(self.label),i)))
        ...
        for i in range(self.train_class_num+1):
            print(i,end="\t")
            for j in range(self.train_class_num+1):
                p = ((a==matric[j]) & (b==matric[i])).sum() 
                p_all = (b==matric[i]).sum()

                print(round(float(p)/(float(p_all)+float("1e-10")), 3),end="\t")
